...
$sqql="UPDATE `recruit` SET `occupation_id`=?,`location_id`=?,
`working_time`=?,`education`=?,`experience`=?,salary=?".
"WHERE id=?";
$stth=$db->prepare($sqql);
$stth->execute(array($occupation_id,$location_id,$worktime,$education,$exper_time,$salary,$r_id));
unset($_SESSION['recruit_id']);
