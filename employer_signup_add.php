...
$sql="SELECT  recruit.id ,occupation.occupation ,location.location,recruit.working_time,recruit.education,recruit.experience,recruit.salary
	FROM  recruit INNER JOIN occupation INNER JOIN location WHERE recruit.occupation_id=occupation.id AND recruit.location_id=location.id ORDER BY recruit.id";
$sth= $db->prepare($sql);
$sth->execute();	
...
?>	 
<form action="employer_signup_save.php" method="POST">
       <tr>
	   <td></td>
	   <td><select name="occupation" class="form-control">
	  <?php	$sql= "SELECT occupation from occupation";
		    $sth = $db->prepare($sql);
		    $sth->execute();
				while($result=$sth->fetchObject()){
				echo '<option>'.$result->occupation.'</option>';
			}	
      ?>
      </select></td>
 	  <td><select name="location" class="form-control">
	  <?php	$sql2= "SELECT location from location";
      ...
      ?>
	   ...
	   <td><button class="btn btn-success" type="submit">Save</button></form><form action="employer_login.php">
